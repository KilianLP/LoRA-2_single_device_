...
class Attention_LoRA(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.lora_backbone_aq = nn.Linear(args.dim, self.r, bias=False)
        self.lora_backbone_bq = nn.Linear(self.r, args.dim, bias=False)
        
        self.experts_aq = nn.ModuleList() 
        self.experts_bq = nn.ModuleList() 
        
        for exp in range(self.n_expert):
            self.experts_aq.append(nn.Linear(args.dim, self.r, bias=False))
        ...
        self.experts_ao = nn.ModuleList() 
        self.experts_bo = nn.ModuleList() 
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        h = x + self.attention(self.attention_norm(x), expert, start_pos, freqs_cis, mask)
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, expert: torch.Tensor,):
        ...
        for layer in self.layers:
            h = layer(h, expert, start_pos, freqs_cis, mask)
        h = self.norm(h)
